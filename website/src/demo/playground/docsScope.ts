 ...
 *
 * It shares the administration's template on purpose: two scopes, one chrome,
 * is exactly the shape of an application whose back office and customer portal
 * look alike but hold different resources. `AdminLayout` is handed straight to
 * `decoratorComponent` here, with no logo and nothing in the top bar — that is
 * the whole of what a scope needs to have a shell.
 */
