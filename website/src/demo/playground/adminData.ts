 ...
 * and the catalogue it sells. Three areas rather than one collection, because
 * that is what a scope is for — an administration is a menu of resources, and
 * a single list would never show it.
 *
 ...
 * and the changes survive a reload.
 */
...
/**
 * Writes the back office fixtures, once per browser session.
 *
 * Same reasoning as the documentation fixtures: only the first call writes, so
 * the edits made in the playground are what the next screen shows. Reloading
 * the tab starts from these rows again.
 */
export function seedAdminData(): void {
  if (seeded) return
  seeded = true

  setInStorage(USERS_ID, collection(USERS_ID, identify(USERS_ID, USERS)))
  setInStorage(POSTS_ID, collection(POSTS_ID, identify(POSTS_ID, POSTS)))
  setInStorage(COMMENTS_ID, collection(COMMENTS_ID, identify(COMMENTS_ID, COMMENTS)))
  setInStorage(PRODUCTS_ID, collection(PRODUCTS_ID, identify(PRODUCTS_ID, PRODUCTS)))
  setInStorage(ORDERS_ID, collection(ORDERS_ID, identify(ORDERS_ID, ORDERS)))
}
